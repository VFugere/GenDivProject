...
function compare_pairwise(species_seqs::Matrix{Int})
    ...
    for iter = 1:(num_seqs-1)
        for iter_sub = (iter + 1):num_seqs # For each pairwise combination of sequences
            ...
            common_sites = vec(minimum(pair_seqs, 1) .> 0) #Ensure that only sites existing in both sequences are compared (i.e. ignore loci missing from the alignment)
            if sum(common_sites) == 0.
                push!(num_per_bp, 0.)
            else
                num_segregating = sum(diff(pair_seqs[:, common_sites], 1) .!= 0)
